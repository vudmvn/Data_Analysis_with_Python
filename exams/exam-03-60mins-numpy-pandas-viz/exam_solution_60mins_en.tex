...
% =========================================================================
\section*{PART II: APPLIED CODING SOLUTIONS \& GRADING RUBRIC (4.0 pts)}

\subsection*{Problem 1: Quantitative E-Commerce Performance Pipeline (2.0 pts)}
...
# [BLANK 3] (0.4 pt)
cust_df, on='cust_id', how='left'

# [BLANK 4] (0.4 pt)
['city', 'tier'])['net_rev'].agg(['sum', 'count']

# [BLANK 5] (0.4 pt)
('net_rev', 'sum'), ascending=False
\end{lstlisting}

\subsubsection*{Grading Criteria \& Scoring Rubric for Problem 1}
\begin{itemize}[leftmargin=*, itemsep=4pt]
  \item \textbf{[BLANK 1] (0.4 pt total):}
  \begin{itemize}
    \item \textbf{0.2 pt:} Accurate 2D slice indexing: \texttt{tx\_data[:, 0]}, \texttt{tx\_data[:, 1]}, \texttt{tx\_data[:, 2]}, \texttt{tx\_data[:, 3]}. (Deduct 0.1 pt if student wrote 1D index like \texttt{tx\_data[0]}).
    \item \textbf{0.2 pt:} Correct algebraic net revenue formula respecting parentheses and order of operations.
    \item \textit{Alternative:} \lstinline[basicstyle=\ttfamily\scriptsize]!(tx_data[:, 0] * tx_data[:, 1] * (1 - tx_data[:, 2])) - tx_data[:, 3]! expanded algebraically is fully accepted (0.4 pt).
  \end{itemize}

  \item \textbf{[BLANK 2] (0.4 pt total):}
  \begin{itemize}
    \item \textbf{0.2 pt:} Correct conditional with bitwise OR \texttt{|} and proper parentheses around individual boolean clauses: \texttt{(net\_revenue >= 250000) | (tx\_data[:, 1] >= 4)}. Deduct 0.1 pt if Python keyword \texttt{or} was used (causes \texttt{ValueError} on arrays).
    \item \textbf{0.2 pt:} Exact string labels \texttt{'Wholesale'} and \texttt{'Retail'} in true and false branches.
  \end{itemize}

  \item \textbf{[BLANK 3] (0.4 pt total):}
  \begin{itemize}
    \item \textbf{0.2 pt:} Target merge table \texttt{cust\_df} and join key \texttt{on='cust\_id'} specified.
    \item \textbf{0.2 pt:} Join type \texttt{how='left'} specified.
  \end{itemize}

  \item \textbf{[BLANK 4] (0.4 pt total):}
  \begin{itemize}
    \item \textbf{0.2 pt:} Grouping keys list \texttt{['city', 'tier']} (or tuple \texttt{('city', 'tier')}).
    \item \textbf{0.2 pt:} Target column selection \texttt{['net\_rev']} chained with aggregation \texttt{.agg(['sum', 'count'])}.
  ...
  \item \textbf{[BLANK 5] (0.4 pt total):}
  \begin{itemize}
    \item \textbf{0.2 pt:} MultiIndex column tuple specification \texttt{('net\_rev', 'sum')} or \texttt{[('net\_rev', 'sum')]}.
    \item \textbf{0.2 pt:} Sorting direction parameter \texttt{ascending=False}.
  \end{itemize}
\end{itemize}
...
\subsection*{Problem 2: Multi-Channel Sales Dashboard \& Risk Analytics (2.0 pts)}
...
# [BLANK 7] (0.6 pt)
data=df_sales, x='payment', y='amount', hue='is_fraud', palette='Set2', ax=axes[0]

# [BLANK 8] (0.4 pt)
rolling(window=4).mean()   # or: rolling(4).mean()
